...
export default function ProfitabilityCalculator({
  ...
  onUpdate: (data: ProfitabilityData) => void
}) {
  ...
  // Mettre à jour les états locaux lorsque les données externes changent
  useEffect(() => {
    if (data) {
  ...
  // Calculer les revenus et profits et mettre à jour les données
  useEffect(() => {
  ...
  }, [platforms, monthlyRent, onUpdate])

  // Gérer le changement du loyer mensuel
  const handleRentChange = (value: number) => {
    ...
    const updatedPlatforms = platforms.map((platform) => {
      const daysPerMonth = (platform.occupancyRate / 100) * 30
      const monthlyRevenue = platform.nightlyRate * daysPerMonth
      const profit = monthlyRevenue - value
      return { ...platform, monthlyRevenue, profit }
    })
  ...
  // Gérer le changement des valeurs de plateforme
  const handlePlatformChange = (index: number, field: keyof Platform, value: number) => {
    ...
    // Calculer immédiatement les revenus et profits
    const platform = updatedPlatforms[index]
    const daysPerMonth = (platform.occupancyRate / 100) * 30
    const monthlyRevenue = platform.nightlyRate * daysPerMonth
    const profit = monthlyRevenue - monthlyRent

    // Mettre à jour la plateforme avec les nouvelles valeurs calculées
    updatedPlatforms[index] = { ...platform, monthlyRevenue, profit }
